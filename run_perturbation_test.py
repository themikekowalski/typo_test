...
def eval_perturb(input_a,input_b):
    ...
    return sq_error, acc, output_b["score"], output_b["label"]
...
for sentence in test_dataset:
    ...
    sq_error, acc, perturb_score, b_label = eval_perturb(sentence, sentence_mod)
    mse += (1/n) * sq_error
    total_acc += (1/n) * acc
    if acc == False:
        interesting_cases.append((sentence, sentence_mod, perturb_score, b_label))
...
df = pd.DataFrame(to_report, columns=["Original", "Perturbed", "Model confidence", "Label"])
